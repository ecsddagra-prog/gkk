    ...
    const subscription = supabase
      ...
      .on('postgres_changes', {
        ...
        if (payload.new.status === 'completed') {
          toast.success('Booking completed! Please rate your experience.')
        }
      })
      .subscribe()
  ...
  const loadBooking = async () => {
    try {
      ...
      if (data?.rating) {
        ...
        setCriteriaRatings({
          behavior: data.rating.behavior_rating || 0,
          nature: data.rating.nature_rating || 0,
          work_knowledge: data.rating.work_knowledge_rating || 0,
          work_quality: data.rating.work_quality_rating || 0,
          punctuality: data.rating.punctuality_rating || 0
        })
        setReviewImages(data.rating.review_photos || [])
      }
    } catch (error) {
            ...
            {booking.status === 'completed' && (
              <div className="bg-white rounded-lg shadow-sm p-6">
                <h2 className="text-xl font-bold mb-6">
                  {booking.rating ? 'Your Review' : 'Rate Your Experience'}
                </h2>

                {booking.rating ? (
                  <div className="space-y-6">
                    <div className="flex items-center gap-4">
                      <div className="text-4xl font-bold text-gray-900">{booking.rating.rating.toFixed(1)}</div>
                      <div>
                        {renderStars(booking.rating.rating, null)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
                      {[
                        { label: 'Behavior', value: booking.rating.behavior_rating },
                        { label: 'Nature', value: booking.rating.nature_rating },
                        { label: 'Work Knowledge', value: booking.rating.work_knowledge_rating },
                        { label: 'Work Quality', value: booking.rating.work_quality_rating },
                        { label: 'Punctuality', value: booking.rating.punctuality_rating },
                    ...
                    {booking.rating.review_text && (
                      <div>
                        ...
                        <p className="text-gray-700 bg-gray-50 p-4 rounded-lg">{booking.rating.review_text}</p>
                      </div>
                    )}

                    {booking.rating.review_photos && booking.rating.review_photos.length > 0 && (
                      <div>
                        <h4 className="font-medium mb-2">Photos</h4>
                        <div className="flex gap-2 overflow-x-auto pb-2">
                          {booking.rating.review_photos.map((url, idx) => (
                  ...
                  <form onSubmit={submitRating} className="space-y-6">
                    ...
                    <div className="text-center p-6 bg-blue-50 rounded-xl">
                      <label className="block text-lg font-medium mb-2 text-blue-900">How would you rate the service overall?</label>
